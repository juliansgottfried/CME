...
Find infectable taxa1. Then get proportion of taxa2 within taxa1 that are infected
...
```{r}
fits$cme[with(pars[fits$idx, ], mu*p == max(mu*p))]
fits$cme[with(pars[fits$idx, ], beta == max(beta))]
fits$cme[with(pars[fits$idx, ], beta == 0 & p == min(p))]
